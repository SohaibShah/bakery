    ...
    app.get("/api/order", (req, res) => {
        if (db && req.query.id) {
            db.get(`select * from \`Order\` where OrderID = ?`, [req.query.id], (err, row) => {
                if (err) { res.status(401); res.json({ error: err.message }) }
    ...
    app.post('/api/orders/add', async (req, res) => {
        if (db && req.body) {
                ...
                db.run('insert into `Order` (CustomerID, OrderDate, Status) values (?, ?, ?)',
                    ...
                    (err) => {
                        if (err) { res.status(401); res.json({ error: err.message }) }
                            ...
                            else { res.status(401); res.json({ error: err.message }) }
                        })
                    })
            } else {
            ...
            const insertOrderDetail = () => {
                ...
                else { res.status(401); res.json({ error: 'Something went wrong!' }) }
            }
        } else { res.status(401); res.json({ error: 'Invalid attempt!' }) }
    })
...
// Customers
{
        ...
        if (db && req.query.id) {
            db.get('select * from `Customer` where CustomerID = ?', [req.query.id], (err, row) => {
                if (!err) {
                    ...
                    { res.status(401); res.json({ error: err.message }) }
                }
            })
        } else { res.status(401); res.json({ error: 'Invalid attempt!' }) }
    })
    ...
    app.post('/api/customers/add', (req, res) => {
        if (db && req.body) {
            ...
            { res.status(401); res.json({ error: 'Invalid attempt!' }) }
        }
    })
